...
class Produto:
    def __init__(self, id_produto=None, nome_produto="", preco=0.0, datadecriacao=None, tamanho=None , nicho =None , id_filial = None):
    ...
    def inserir(self, conexao):
        ...
        try:
            cursor.execute(
                "INSERT INTO Produto (NomeProduto, Preco, Tamanho, Datadecriacao, Nicho, ID_filial) VALUES (%s, %s, %s, %s, %s, %s)",
                (self.nome_produto, self.preco, self.tamanho, self.datadecriacao, self.nicho , self.id_filial)
            )
            conexao.commit()
        except Exception as e:

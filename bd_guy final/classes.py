...
class ItemVenda:
    ...
    def inserir(self, conexao):
        ...
        cursor = conexao.cursor()
        try:
            query = "INSERT INTO Item_Venda (NomeProduto, Quantidade, ID_Vendedor, ID_Cliente) VALUES (%s, %s, %s, %s)"
            cursor.execute(query, (self.nome_produto, self.quantidade, self.id_vendedor, self.id_cliente))
            conexao.commit()
    ...
    def deletar(self, conexao, **filtros):
       
        cursor = conexao.cursor()
        try:
            if not filtros:
                print("Nenhum filtro mandadoS. Especifique pelo menos um deles para deletar.")
                return

            
            condicoes = " AND ".join([f"{campo} = %s" for campo in filtros.keys()])
            valores = tuple(filtros.values())

            query = f"DELETE FROM Item_Venda WHERE {condicoes}"
            cursor.execute(query, valores)
            conexao.commit()
            print(f"campod onde {filtros} foram deletados com sucesso.")
        except Exception as e:
